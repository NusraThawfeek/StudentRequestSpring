...
public interface StudentReqRepo extends JpaRepository<StudentReq, Integer> {
	...
	@Query(value = " SELECT distinct(r.rid),r.description,r.current_status,r.entereddatetime,r.is_sent_to_fac_board\r\n"
			+ " FROM student_req r, commented c\r\n"
			+ " WHERE  r.rid=c.rid and c.rid not in(SELECT r.rid\r\n"
			+ " FROM student_req r, commented c\r\n"
			+ " WHERE  r.rid=c.rid and c.uid=:uid)", nativeQuery = true)
